...
public class HoloStreamEventDecoderTest {
  private static final String AUCTION_TOPIC = "nexmark-auction";
  private static final String BID_TOPIC = "nexmark-bid";
  private static final byte[] AUCTION_FIXTURE =
      hex(
          "d00f094b4d4c5845544c554a0b4a544c4144574354515142ca01dc0480897a80a98ea68d1dd00f140000");
  private static final byte[] BID_FIXTURE = hex("d40fd00f8acf9b1f80a4e8030000");
  ...
  private static HoloStreamEventDeserializer configuredDeserializer() {
    Map<String, Object> config = new HashMap<>();
    config.put(HoloStreamEventDeserializer.AUCTION_TOPIC_CONFIG, AUCTION_TOPIC);
    config.put(HoloStreamEventDeserializer.BID_TOPIC_CONFIG, BID_TOPIC);
  ...
  public void decodesGoAuctionFixtureAndConvertsNanoseconds() throws Exception {
    HoloStreamEventDecoder.DecodedEvent decoded =
        HoloStreamEventDecoder.decode(
            HoloStreamEventDecoder.EventType.AUCTION, AUCTION_FIXTURE);
  ...
  @Test
  public void oneDeserializerSelectsAuctionAndBidFromTopic() {
    HoloStreamEventDeserializer deserializer = configuredDeserializer();

    assertTrue(deserializer.deserialize(AUCTION_TOPIC, AUCTION_FIXTURE).newAuction != null);
    assertTrue(deserializer.deserialize(BID_TOPIC, BID_FIXTURE).bid != null);
  }
  ...
  public void kafkaKeyDeserializerNormalizesNullKey() {
    ...
    assertArrayEquals(new byte[0], deserializer.deserialize(BID_TOPIC, null));
    assertArrayEquals(
        new byte[] {1, 2, 3}, deserializer.deserialize(BID_TOPIC, new byte[] {1, 2, 3}));
  }

  @Test(expected = SerializationException.class)
  public void kafkaDeserializerRejectsUnknownTopic() {
    configuredDeserializer().deserialize("unexpected", BID_FIXTURE);
  }

  @Test(expected = SerializationException.class)
  public void kafkaDeserializerRejectsWrongTupleTypeForTopic() {
    configuredDeserializer().deserialize(BID_TOPIC, AUCTION_FIXTURE);
  }
  ...
  public void timestampPolicyUsesAlreadyDeserializedEvent() {
    ...
    TimestampPolicy<byte[], Event> policy =
        new HoloStreamTimestampPolicyFactory()
            .createTimestampPolicy(new TopicPartition(BID_TOPIC, 2), Optional.empty());
  ...
  public void rejectsTrailingBytes() throws Exception {
    HoloStreamEventDecoder.decode(
        ...
        hex("d40fd00f8acf9b1f80a4e803000001"));
  }
}
